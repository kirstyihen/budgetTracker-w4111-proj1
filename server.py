...
def user_profile(name):
    ...
    #query = text('SELECT a.balance FROM')
    #params = {'uni': uni}
    #result = g.conn.execute(query, params)
    #g.conn.commit()
    #for j in result:
    ...
    return render_template("user_profile.html", info={'uni': uni, 'name': name, 'username': username, 'password': password, 
            'schoolcode': schoolcode, 'schoolname': schoolname })
    #'balance': balance, 'swipes': swipes,'dining_dollars': dining_dollars,'points': points, 'flex': flex})
